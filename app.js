...
async function loadTimeseries(parameter, zoneId) {
  ...
  if (!state.timeseriesCache.has(cacheKey)) {
    ...
    state.timeseriesCache.set(cacheKey, payload);
  }
...
function renderDetailCharts(zoneSeries, record) {
  const baseSeries = Array.isArray(zoneSeries) ? zoneSeries : (zoneSeries.records ?? []);
  const dailySeries = zoneSeries.daily_records ?? aggregateDailySeries(baseSeries);
  const focusSeries = zoneSeries.recent_records ?? getFocusWindow(baseSeries, 14);
